...
import FeaturesAnimationVideo from "../assets/videos/funnel-animation.mp4";
...
export default function Animation() {
  ...
  const animationTextBoxes = [
    ...
    {
      id: 1,
      title: "2.Question Relevance Assessment",
      paragraph:
  ...
  return (
    <figure className='animation-video__wrapper flex relative ultra-small:mb-6 md:mb-0 mx-auto'>
      <video className='animation-video h-auto w-full' autoPlay loop muted>
        <source src={FeaturesAnimationVideo} type='video/mp4' />
      </video>
      <figcaption className='animation-text__boxes absolute top-0 left-0 z-10 w-full h-full flex flex-col justify-between very-large:pl-[26%] very-large:pr-[25%] medium-large:pl-[20%] small-large:pl-[16%] custom-between:pl-[12%] md:pl-[0%] md:py-[33%] small-large:pr-[14%] very-small:py-[20%] ultra-small:py-[0%]'>
        {animationTextBoxes.map(({ id, title, paragraph }) => (
          <button
            ...
            key={id}
            className={`animation-text__boxes max-w-min overflow-hidden text-left py-4 px-6 rounded border border-solid border-black ultra-small:text-wrap ${
              isDarkMode
