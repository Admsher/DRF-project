...
function Questions({
  ...
  const [view, setView] = useState(
    Array.from({ length: totalQuestions.length }, () => false)
  );
  ...
  const handleFinalize = () => {
    const remainingQuestions = totalQuestions.filter(
    ...
    <div className="h-[360px] bg-ltblue rounded-md border-2 border-dkblue">
      ...
      <div className="p-4 h-[80%] overflow-hidden overflow-y-scroll rounded-md">
        {totalQuestions.map((question, index) => (
          <div key={question.id}>
            {/* Question box */}
            {/* Check how the border changes for the below based on view boolean value */}
            <div
              key={question.id}
              className={`bg-mdblue text-white h-full p-3 flex items-center px-3 mt-2 justify-between rounded-md ultrasmall:flex-col md:flex-row ${
                view[index] && "rounded-bl-none rounded-br-none"
              }`}
            >
              {" "}
              {/* Check box and question */}
              ...
              <div className="flex justify-between gap-5 w-[20%]">
                <button
                ...
                {/* Use switch to specify difficulty*/}
                {question.difficulty === "easy" && (
                  <div className="bg-[#39cb3e]  rounded-md px-1">Easy</div>
                )}
                {question.difficulty === "medium" && (
                  <div className="bg-[#d653f4]  rounded-md px-1">Medium</div>
                )}
                {question.difficulty === "hard" && (
                  <div className="bg-[#FF9800]  rounded-md px-1">Hard</div>
                )}
                {question.difficulty === "veryhard" && (
                  <div className="bg-[#F44336]  rounded-md px-1">Very Hard</div>
                )}
              </div>
            </div>
            {/* Answer Box */}
